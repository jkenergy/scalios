 ...
 * $HeadURL: http://10.211.55.3/svn/repos/ertcs/rtos/trunk/embedded/src/COM/recmsg.c $
 * 
 * Target CPU: 		Generic
...
StatusType ReceiveMessage(MessageIdentifier Message, ApplicationDataRef DataRef)
{
	...
	COM_TRACE_ON();
	COM_TRACE_CODE(COM_TRACE_RECEIVE_REGULAR_MESSAGE);
	...
	if(!COM_STARTED()) {
		...
		assert(Message->cb.receiver->driver->recv);
		rc = Message->cb.receiver->driver->recv(Message->cb.receiver, DataRef);
	}
#else	
	rc = Message->cb.receiver->driver->recv(Message->cb.receiver, DataRef);
#endif
	if(rc != E_OK && COM_ERROR_HOOK_CALLABLE()) {		/* $Req: artf1265 $ */
		...
		COM_ERRORHOOK_2(ReceiveMessage, rc, Message, DataRef);
	...
	COM_TRACE_ON();
	COM_TRACE_CODE(COM_TRACE_RECEIVE_REGULAR_MESSAGE_FINISH);
	COM_TRACE_OFF();
