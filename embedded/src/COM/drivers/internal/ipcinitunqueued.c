 ...
 * $HeadURL: http://10.211.55.3/svn/repos/ertcs/rtos/trunk/embedded/src/COM/drivers/internal/ipcinitunqueued.c $
 * 
 * Target CPU: 		Generic
...
/* Driver "init receiver" function. The 'device type' is local IPC. Only works for an unqueued receive message.
 * There is only one instance of the IPC pseudo-device, so no "device" is specified in the concrete driver-specific receivercb.
 */
StatusType com_driver_internal_init_receiver_unqueued(com_receiverh receiver, ApplicationDataRef data)
{
	...
	OS_BLOCK_COPY(internal_recv->message_size, data, internal_recv->msg_data);		/* $Req: artf1263 $ */

	*internal_recv->flag = COM_FALSE;
	
	IPC_UNLOCK_BUFFER(internal_recv->guard);
		
	return E_OK;	/* $Req: artf1278 $ */
}
