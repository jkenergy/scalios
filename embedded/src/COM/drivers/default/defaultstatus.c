 ...
 * $HeadURL: http://10.211.55.3/svn/repos/ertcs/rtos/trunk/embedded/src/COM/drivers/default/defaultstatus.c $
 * 
 * Target CPU: 		Generic
...
/* Driver "status" function. The 'device type' is local IPC. Only works for a queued receive message.
 * There is only one instance of the IPC pseudo-device, so no "device" is specified in the concrete driver-specific receivercb.
 */

StatusType com_driver_default_status(com_receiverh receiver)
{
